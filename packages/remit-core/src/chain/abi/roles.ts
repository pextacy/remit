/**
 * Zodiac Roles Modifier v2 ABI — the ABI of the mastercopy actually deployed at
 * `ROLES_MASTERCOPY` on every chain we run on.
 *
 * Source: gnosisguild/zodiac-modifier-roles, `packages/evm/mastercopies.json`,
 * entry `Roles` → `2.1.0`, at tag `zodiac-roles-sdk-v4.1.3`
 * (commit 820e5bc975d1817bdd4bc4a95226f553f7b67b68). Verified 2026-09-13.
 *
 * Copied verbatim from that file — not hand-written, not recalled. The `error`
 * entries are the reason this is the whole ABI rather than the four functions we
 * call: G2 decodes reverts against them, and an undecoded `0x…` is a failed
